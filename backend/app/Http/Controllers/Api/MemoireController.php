...
class MemoireController extends Controller
{
    ...
    public function fichierPublic(Memoire $memoire)
    {
        ...
        $nom = $memoire->nom_original_fichier ?: (Str::slug($memoire->titre) . '.pdf');

        return Storage::disk('s3')->response($memoire->fichier_memoire, $nom);
    }
    ...
    public function fichePreuveSignee(Memoire $memoire)
    {
        ...
        $nom = Str::slug($memoire->titre) . '-preuve.pdf';

        return Storage::disk('s3')->response($memoire->fichier_preuve, $nom);
    }
    ...
    public function telechargerPublic(Memoire $memoire)
    {
        ...
        return Storage::disk('s3')->download($memoire->fichier_memoire, $nom);
    }
    ...
    public function monFichier(Request $request, Memoire $memoire, string $type)
    {
        ...
        $nom = $type === 'memoire'
            ? ($memoire->nom_original_fichier ?: (Str::slug($memoire->titre) . '-memoire.pdf'))
            : (Str::slug($memoire->titre) . '-preuve.pdf');

        return Storage::disk('s3')->response($chemin, $nom);
    }
    ...
    public function monTelechargement(Request $request, Memoire $memoire, string $type)
    {
        ...
        return Storage::disk('s3')->download($chemin, $nom);
    }
