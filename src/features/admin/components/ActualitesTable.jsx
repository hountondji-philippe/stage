...
export default function ActualitesTable({ actualites, onEdit, onDelete }) {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
      {/* overflow-x-auto : le tableau défile horizontalement sur mobile au lieu
          de déborder de l'écran. table-fixed + largeurs de colonnes en % :
          sans ça, "truncate" sur la description ne sert à rien (une colonne
          sans largeur définie s'étire pour accueillir tout son contenu). */}
      <div className="overflow-x-auto">
        <table className="w-full min-w-[560px] table-fixed text-left text-sm">
          <thead className="bg-gray-50 text-xs uppercase tracking-wide text-gray-500">
            <tr>
              <th className="w-[55%] px-6 py-3 font-semibold">Actualité</th>
              <th className="w-[25%] px-6 py-3 font-semibold">Date de publication</th>
              <th className="w-[20%] px-6 py-3 text-right font-semibold">Actions</th>
            </tr>
          </thead>
            ...
            {actualitesPage.map((a) => {
              ...
              return (
                <tr key={a.id} className="hover:bg-gray-50/60">
                  ...
                  <td className="truncate px-6 py-4 text-gray-500">{formatDate(a.date_publication)}</td>
                  <td className="px-6 py-4">
