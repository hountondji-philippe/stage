...
export default function FilieresTable({ filieres, onAjouterSousFiliere, onEdit, onDelete }) {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
      {/* overflow-x-auto + table-fixed + largeurs en % : même correctif que
          ActualitesTable.jsx — sans ça le bouton "Ajouter sous-filière" et la
          description débordent de l'écran sur mobile au lieu de défiler. */}
      <div className="overflow-x-auto">
        <table className="w-full min-w-[640px] table-fixed text-left text-sm">
          <thead className="bg-gray-50 text-xs uppercase tracking-wide text-gray-500">
            <tr>
              <th className="w-[22%] px-6 py-3 font-semibold">Nom</th>
              <th className="w-[33%] px-6 py-3 font-semibold">Description</th>
              <th className="w-[45%] px-6 py-3 text-right font-semibold">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {filieresPage.map((filiere) => (
              <tr key={filiere.id} className="hover:bg-gray-50/60">
                <td className="truncate px-6 py-4 font-semibold text-[var(--color-text)]">
                  {filiere.nom}
                </td>
                <td className="truncate px-6 py-4 text-gray-500">{filiere.description || "—"}</td>
                <td className="px-6 py-4">
                  <div className="flex items-center justify-end gap-2">
                    <button
                      onClick={() => onAjouterSousFiliere(filiere)}
                      className="flex shrink-0 items-center gap-1.5 whitespace-nowrap rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-semibold text-[var(--color-primary)] hover:bg-[var(--color-primary)]/5"
                    >
